...
Then, run the `run.sh` script with `SP22` as the only argument to get the new cleaned CSV files.

## Some Remarks
- There may be many CSV files with inaccurate data (e.g. CSE 110, CSE 8A). This is because I was testing the tracker and associated wrapper functions, which had many logic errors at the time.
